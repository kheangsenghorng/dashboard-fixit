...
import { decodeId, encodeId } from "../../../utils/hashids"; // ✅ added encodeId
...
export default function CategoryPage() {
  ...
  const { fetchTypesCategory, typeCategory, setActiveTypes } = useTypeStore();
      ...
      <main className="max-w-[1300px] mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Found {typeCategory.length} sub-categories available
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-10">
          {typeCategory.map((item) => (
            <Link
              key={item.id}
              href={`/type/${encodeId(item.id)}`} // ✅ FIXED: encode the type id
