...
export default function CreateUserPage() {
  ...
  const roles = [
    ...
    { id: "provider", label: "Provider", desc: "access", icon: ShieldCheck },
  ];
  ...
  const handleManualSubmit = async (e) => {
    ...
    // 1. Create a promise for the toast
    const createPromise = createUser(formData);
    ...
    toast.promise(createPromise, {
      ...
      success: {
        render() {
          ...
          return `User ${formData.name} created successfully!`;
        },
      },
      error: {
        render({ data }) {
          // Extract specific error messages for the red text under inputs
          const apiErrors = data.response?.data?.errors;
          if (apiErrors) setErrors(apiErrors);
          return data.response?.data?.message || "Failed to create user";
        }
      }
    });
  };
  ...
  const handleExcelImport = async (fileData) => {
    try {
      ...
      setErrors({ file: [e.response?.data?.message || "Import failed"] });
    }
  };

  return (
    <div className="min-h-screen bg-[#F8FAFC] p-6 text-slate-900">
      {/* Toast Container - Place this once in your app */}
      <ToastContainer position="top-right" autoClose={4000} />

      {/* Header Area */}
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <button onClick={() => router.back()} className="group flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-all mb-2 font-medium">
            <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" /> 
            Back to Directory
          </button>
          <h1 className="text-3xl font-extrabold tracking-tight">Add New User</h1>
        </div>
