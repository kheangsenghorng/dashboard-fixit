...
import { 
  ...
  Fingerprint, Briefcase, Globe
} from 'lucide-react';
import Image from "next/image";

const EditUserPage = ({ userData, onSave, onBack }) => {
  const [formData, setFormData] = useState({
    ...
    avatar: userData?.avatar || null, // Assuming avatar might be in userData
  });
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
                <div className="relative group w-32 h-32 mx-auto mb-6">
                    <div className="w-full h-full rounded-full overflow-hidden bg-slate-100 border-4 border-white shadow-xl flex items-center justify-center">
                        {formData.avatar ? (
                            <Image src={formData.avatar} alt="Profile" fill className="object-cover" unoptimized />
                        ) : (
                            <span className="text-4xl font-bold text-indigo-400">{formData.name?.charAt(0)}</span>
                        )}
                    </div>
                    <button className="absolute bottom-0 right-0 p-2.5 bg-indigo-600 text-white rounded-full border-4 border-white shadow-lg hover:bg-indigo-700 transition-colors">
                        <Camera size={16} />
                    </button>
                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                                        <ArrowLeft size={16} className="-rotate-90 text-slate-400" />
                                    </div>
                                </div>
                            </div>
                        </div>
