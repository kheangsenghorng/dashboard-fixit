...
export default function EditCompany() {
  ...
  const companyId = decodeId(encodedId);
  ...
  useEffect(() => {
    ...
    setGallery({
      existing: Array.isArray(owner.images)
        ? owner.images
  ...
  const processLogo = (file) => {
    ...
    if (file.size > 2 * 1024 * 1024)
      return toast.error("Logo must be under 2MB"); // ✅ Error Toast
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.lat || !formData.lng)
      return toast.error("Please pin the location on the map."); // ✅ Error Toast
    ...
    const success = await updateOwner(owner.id, data);
    if (success) {
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">
            {formData.business_name || "Edit Profile"}
          </h1>
        </div>
                ...
                <div className="relative group">
                  ...
                  <label className="absolute -bottom-2 -right-2 p-3 bg-indigo-600 text-white rounded-2xl shadow-xl cursor-pointer hover:scale-110 active:scale-90 transition-all">
                    ...
                    <input
                      ...
                      className="hidden"
                      accept="image/*"
                      onChange={(e) => processLogo(e.target.files?.[0])}
                    />
                  </label>
                </div>
                <div className="flex-1 w-full space-y-6">
                  <div className="group">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-b-2 border-slate-100 py-2 text-2xl font-bold text-slate-800 focus:outline-none focus:border-indigo-600 transition-all"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 outline-none transition-all"
                  >
                ...
                <div className="md:col-span-2 space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-sm font-bold text-slate-700 outline-none resize-none transition-all"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between px-2">
                ...
                <span className="text-[11px] font-bold text-indigo-600 bg-indigo-50 px-4 py-1.5 rounded-full">
                  {gallery.existing.length + gallery.newFiles.length} Assets
                </span>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setGallery((prev) => ({
                          ...prev,
                          existing: prev.existing.filter((_, idx) => idx !== i),
                        }))
                      }
                      className="absolute inset-0 bg-rose-600/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2"
                    >
                      <Trash2 size={24} />
                ...
                {gallery.newFiles.map((item, i) => (
                  <div
                    key={`new-${i}`}
                    className="relative aspect-square rounded-[2rem] overflow-hidden border-2 border-indigo-500"
                  >
                    <Image
                      src={item.preview}
                      alt="New Image"
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-2 bg-white text-rose-500 rounded-xl shadow-lg hover:bg-rose-500 hover:text-white transition-colors"
                    >
                      <X size={16} />
                    </button>
                  </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-10">
            ...
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-sm transition-all hover:bg-slate-50">
              ...
              <p className="text-sm text-slate-500 leading-relaxed font-medium">
                Updated geospatial data and gallery visuals increase profile
                engagement by up to 45%.
              </p>
            </div>
            ...
            <button
              ...
              className="w-full py-5 border-2 border-slate-100 rounded-3xl text-[11px] font-black uppercase tracking-widest text-slate-400 hover:bg-white hover:border-slate-200 hover:text-indigo-600 transition-all flex items-center justify-center gap-3 active:scale-95"
            >
              <RefreshCcw size={14} /> Scroll to top
            </button>
          </div>
        </form>
      </main>
