...
const initialServices = [
  { id: 1, name: 'Full House Deep Cleaning', category: 'Cleaning', basePrice: 120.00, duration: '4-6 hrs', status: 'Active', popularity: 'High' },
  { id: 2, name: 'Electrical Socket Repair', category: 'Electrical', basePrice: 45.00, duration: '1 hr', status: 'Active', popularity: 'Medium' },
  { id: 3, name: 'Emergency Pipe Leak', category: 'Plumbing', basePrice: 85.00, duration: '2 hrs', status: 'Active', popularity: 'High' },
];
    ...
    <div className="min-h-screen bg-[#f5f5f7] p-4 md:p-10 pb-20">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <span className="text-gray-400 text-xs font-bold">ID: #{id}</span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { title: "Profile Views", value: "1,204", icon: Eye, color: "text-blue-500" },
            { title: "Services", value: initialServices.length, icon: Layers, color: "text-indigo-500" },
            { title: "Bookings", value: "312", icon: TrendingUp, color: "text-emerald-500" },
            { title: "Docs", value: owner.documents?.length || 0, icon: FileText, color: "text-amber-500" },
          ].map((card, i) => (
            <motion.div
              key={i}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            <div className="flex justify-between items-center border-b border-gray-50 pb-6">
               ...
               <a href={owner.map_url} target="_blank" className="text-indigo-600 text-xs font-bold flex items-center gap-1">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-6">
                    <DetailItem icon={<MapPin className="text-indigo-500" />} label="Address" value={owner.address} />
                    <DetailItem icon={<Phone className="text-indigo-500" />} label="Phone Number" value={owner.user?.phone || 'N/A'} />
                    <DetailItem icon={<User className="text-indigo-500" />} label="Owner Name" value={owner.user?.name} />
                </div>
                <div className="bg-gray-50 rounded-[30px] p-6 flex flex-col justify-center items-center text-center">
                    <p className="text-xs font-bold text-gray-400 uppercase mb-2">Member Since</p>
          ...
          >
            <div>
                ...
                <p className="text-gray-400 text-sm mb-8">Process business application status</p>
                ...
                {isPending ? (
                  <div className="space-y-4">
                    ...
                    >
                      {reviewLoading ? "Processing..." : <><CheckCircle2 size={18}/> Approve Business</>}
                    </button>
                  ...
                  <div className="bg-emerald-50 border border-emerald-100 rounded-[30px] p-8 text-center">
                    ...
                    <p className="font-bold text-emerald-800">Business Verified</p>
                    <p className="text-xs text-emerald-600 mt-1">Status: {owner.status}</p>
                  </div>
                )}
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50">
                {initialServices.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase())).map((service) => (
                  <tr key={service.id} className="hover:bg-gray-50/50 transition">
                    <td className="px-8 py-6">
                      <p className="text-sm font-bold text-gray-900">{service.name}</p>
                      <p className="text-[10px] text-gray-400 font-bold">{service.duration}</p>
                    </td>
                    <td className="px-8 py-6 text-sm text-gray-600 font-medium">{service.category}</td>
                    <td className="px-8 py-6 text-sm font-bold text-gray-900">${service.basePrice}</td>
                    <td className="px-8 py-6">
                      <span className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-bold uppercase">{service.status}</span>
                    </td>
                    <td className="px-8 py-6">
        ...
        {/* ================= DOCUMENT REVIEW ================= */}
        <motion.div {...fadeUp} className="bg-white rounded-[40px] shadow-sm border border-gray-100 p-8">
          <h2 className="text-xl font-bold mb-8">Registration Documents</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {owner.documents?.length ? owner.documents.map((doc, i) => (
              <div key={i} className="group border-2 border-gray-50 rounded-[32px] p-6 hover:border-indigo-100 transition-all bg-white relative">
                <FileText className="text-gray-300 group-hover:text-indigo-500 transition-colors mb-4" size={32} />
                <p className="font-bold text-gray-900">{doc.document_type}</p>
                <p className="text-xs text-gray-400 mt-1 uppercase font-bold tracking-tighter">Status: {doc.status}</p>
                {doc.file_url && (
                  <a href={doc.file_url} target="_blank" className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-indigo-600 group-hover:underline">
        ...
        {/* ================= PHOTO GALLERY ================= */}
        <motion.div {...fadeUp} className="bg-white rounded-[40px] shadow-sm border border-gray-100 p-8">
          <h2 className="text-xl font-bold mb-8 flex items-center gap-2"><Camera size={22} className="text-indigo-600" /> Media Gallery</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {owner.images?.map((img, i) => (
              <img key={i} src={img} className="h-40 w-full object-cover rounded-[24px] hover:scale-105 transition duration-500 shadow-sm" />
            ))}
          </div>
        </motion.div>
