...
const AssignStaffModal = ({
  ...
  const { createServiceBookingProvider } = useServiceBookingProviderStore();
  const { fetchCheckProvidersByOwner } = useProviderStore();
  const [assigningId, setAssigningId] = useState(null);

  const getStaffId = (staff) =>
    staff?.providerId || staff?.provider_id || staff?.provider?.id || staff?.id;

  const getAssignedProviderId = (staff) =>
    staff?.provider_id || staff?.providerId || staff?.provider?.id || staff?.id;
  ...
  const handleAssignStaff = async (staff) => {
    ...
    if (!staffId) {
      console.error("Provider ID not found:", staff);
      return;
    }

    const isAssigned = assignedStaff.some(
      (assigned) => Number(getAssignedProviderId(assigned)) === Number(staffId)
    );
    ...
    try {
      ...
      const createdItem = await createServiceBookingProvider({
          ...
          <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
            {filtered.length > 0 ? (
              filtered.map((staff, index) => {
                ...
                const isAssigned = assignedStaff.some(
                  (assigned) =>
                    Number(getAssignedProviderId(assigned)) === Number(staffId)
                );

                const isAssigning = Number(assigningId) === Number(staffId);
                ...
                return (
                  <button
                    ...
                    className={`w-full group flex items-center justify-between p-4 rounded-[1.5rem] border-2 transition-all text-left ${
                      isAssigning
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
                          {isAssigning ? "Assigning..." : staffSpecialty}
                        </span>
                      </div>
                    </div>
