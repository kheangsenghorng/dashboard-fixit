...
import { 
  ArrowLeft, Search, CheckCircle2, Sparkles, Layers, Plus, X, 
  DollarSign, Clock, ImageIcon, FileText, Layout, ChevronRight, 
  RotateCcw, Zap, Globe, ShieldCheck
} from "lucide-react";
...
export default function CreateServicePage() {
  ...
  const { activeTypes = [], categories = [], fetchActiveCategories, fetchActiveTypes } = useTypeStoreCompany();

  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [formData, setFormData] = useState({
    title: "", description: "", category_id: "", type_id: "",
    base_price: "", duration: "", owner_id: "",
  });
  ...
  if (loading) return <ContentLoader title="Syncing Network" subtitle="Deploying your new service to the marketplace..." />;
        ...
        <nav className="flex items-center justify-between mb-16">
            ...
            <div className="flex items-center gap-6 bg-white/80 backdrop-blur-md px-6 py-2.5 rounded-full border border-slate-200 shadow-sm">
                <div className="flex -space-x-2">
                   {[1,2,3,4].map(i => (
                     <div key={i} className={`w-2.5 h-2.5 rounded-full border-2 border-white ${progress >= i*25 ? 'bg-indigo-600' : 'bg-slate-200'}`} />
                   ))}
                </div>
                <span className="text-xs font-bold text-slate-500 uppercase tracking-tighter">{progress}% Complete</span>
            </div>
        </nav>

        <header className="mb-12">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <span className="text-indigo-600 font-bold text-xs uppercase tracking-[0.2em] mb-3 block">Marketplace Entry</span>
                <h1 className="text-6xl font-black tracking-tight text-slate-900">Create <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500">Service</span></h1>
            </motion.div>
        </header>
            ...
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Define your niche and specialization</p>
                    </div>
                </div>
                {formData.category_id && (
                    <button 
                        type="button" 
                        onClick={() => setFormData(p => ({...p, category_id: "", type_id: ""}))}
            ...
            <div className="p-8">
                <AnimatePresence mode="wait">
                    {!formData.category_id ? (
                        ...
                        <motion.div key="type-grid" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {activeTypes.map(type => (
                                    <button
                                        key={type.id} type="button"
                                        onClick={() => setFormData(p => ({...p, type_id: type.id.toString()}))}
                                    ...
                                    >
                                        ...
                                        <div className="text-left overflow-hidden">
                                            ...
                                            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Select</p>
                                        </div>
                                        {formData.type_id === type.id.toString() && <CheckCircle2 size={18} className="ml-auto text-indigo-600" />}
                                    </button>
                                ))}
                            </div>
                        </motion.div>
                    ...
                    <section className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm space-y-8">
                        ...
                        {authUser?.role === "admin" && (
                            <div className="grid grid-cols-1 gap-2">
                                <label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest px-1">Account Owner</label>
                                <select 
                                ...
                                >
                                    ...
                                    {owners.map(o => <option key={o.id} value={o.id}>{o.business_name}</option>)}
                                </select>
                            </div>
                        )}

                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest px-1">Service Title</label>
                            <input 
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase text-slate-400 tracking-widest px-1">Detailed Description</label>
                            <textarea 
                                rows={4} 
                    ...
                    <section className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <label className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-bold text-xs cursor-pointer hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200">
                                <span className="flex items-center gap-2"><Plus size={14} /> Add Images</span>
                ...
                <div className="lg:col-span-4 space-y-6">
                    <section className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden">
                        <div className="relative z-10 space-y-10">
                            ...
                            <div className="space-y-2">
                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Base Rate</p>
                    ...
                    <div className="bg-indigo-50 p-6 rounded-[2rem] border border-indigo-100">
                        <p className="text-xs text-indigo-700 leading-relaxed italic font-medium">
                            "High-quality descriptions and clear pricing increase booking conversion by up to 40%."
                        </p>
                    </div>
                </div>
            </motion.div>
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] w-full max-w-md px-6">
            ...
            >
                <div className="pl-6 pr-4">
                    <p className="text-[9px] font-black uppercase tracking-widest text-indigo-400">Status</p>
