        ...
        <div className="max-w-[1440px] mx-auto flex items-center justify-between h-20 px-6">
          ...
          <div className="flex items-center gap-10">
            ...
            <nav className="hidden lg:flex items-center gap-8">
              <Link href="/" className="text-[13px] font-bold text-slate-500 hover:text-blue-600 transition">
                Home
              </Link>

              {/* ✅ FIX: Shop now has an href */}
              <Link href="/shop" className="flex items-center gap-1 text-[13px] font-bold text-slate-500 hover:text-blue-600 transition">
              ...
              <div className="relative" ref={categoryDropdownRef}>
                <button
                  onClick={() => { setOpenCategory(!openCategory); setOpenType(false); }}
                  className={`flex items-center gap-1 text-[13px] font-bold transition py-2 border-b-2 ${
                    openCategory ? "text-blue-600 border-blue-600" : "text-slate-500 border-transparent hover:text-blue-600"
                  }`}
                >
                  Categories{" "}
                  <ChevronDown size={14} className={`transition-transform duration-300 ${openCategory ? "rotate-180" : ""}`} />
                </button>
                  ...
                  <div className="absolute left-[-150px] top-full pt-4 w-[850px]">
                    <div className="bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] rounded-[2.5rem] border border-slate-100 flex overflow-hidden animate-in fade-in slide-in-from-top-3 duration-300">
                      <div className="w-[35%] bg-[#F8FAFC] p-10 border-r border-slate-100 flex flex-col">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 mb-4 leading-tight">Shop by <br /> Category</h3>
                        <p className="text-sm text-slate-400 font-medium mb-10 leading-relaxed">
                          Explore our vast registry of products categorized for your needs.
                        </p>
                        <Link href="/categories" className="mt-auto text-blue-600 text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-2 hover:gap-4 transition-all">
                          All Categories <ArrowRight size={16} />
                        </Link>
                      </div>
                      <div className="w-[65%] p-10 grid grid-cols-2 gap-x-10 gap-y-8">
                        {catLoading ? (
                          <div className="col-span-2 flex items-center justify-center py-20 text-slate-400 text-sm italic">Loading categories...</div>
                        ) : (
                          categories?.slice(0, 8).map((cat) => (
                            <Link key={cat.id} href={`/category/${encodeId(cat.id)}`} onClick={() => setOpenCategory(false)} className="flex items-center gap-5 group">
                              <div className="w-14 h-14 shrink-0 bg-slate-100 rounded-2xl flex items-center justify-center group-hover:bg-blue-600 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-blue-200">
                                <img src={cat.icon} className="w-7 h-7 object-contain group-hover:brightness-0 group-hover:invert transition-all" alt={cat.name} />
                              </div>
                              <div className="flex flex-col">
                                <span className="text-[15px] font-bold text-slate-800 group-hover:text-blue-600 transition-colors">{cat.name}</span>
                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">View Collection</span>
                              </div>
                            </Link>
                          ))
                        )}
              ...
              <div className="relative" ref={typeDropdownRef}>
                <button
                  type="button"
                  onClick={() => { setOpenType(!openType); setOpenCategory(false); }}
                  className={`flex items-center gap-1 text-[13px] font-bold transition py-2 border-b-2 ${
                    openType ? "text-blue-600 border-blue-600" : "text-slate-500 border-transparent hover:text-blue-600"
                  }`}
                >
                  Types{" "}
                  <ChevronDown size={14} className={`transition-transform duration-300 ${openType ? "rotate-180" : ""}`} />
                </button>
                  ...
                  <div className="absolute left-[-300px] top-full pt-4 z-50">
                    <div className="bg-white shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] rounded-[2.5rem] border border-slate-100 flex overflow-hidden animate-in fade-in slide-in-from-top-3 duration-300 w-[850px]">
                      <div className="w-[35%] bg-[#FDFCFB] p-10 border-r border-slate-100 flex flex-col">
                        ...
                        <h3 className="text-2xl font-black text-slate-900 mb-4 leading-tight">Explore <br /> Collections</h3>
                        <p className="text-sm text-slate-400 font-medium mb-10 leading-relaxed">
                          Discover products grouped by specific types and unique collections.
                        </p>
                        <Link href="/types" onClick={() => setOpenType(false)} className="mt-auto text-blue-600 text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-2 hover:gap-4 transition-all">
                      ...
                      <div className="w-[65%] p-10">
                        <div className="grid grid-cols-2 gap-x-10 gap-y-8">
                          {typeLoading ? (
                            <div className="col-span-2 py-20 text-center text-slate-400 text-sm italic">Loading types...</div>
                          ) : activeTypes?.length > 0 ? (
                            activeTypes.slice(0, 8).map((type) => (
                              <Link key={type.id} href={`/types/${encodeId(type.id)}`} onClick={() => setOpenType(false)} className="flex items-center gap-5 group cursor-pointer">
                                <div className="w-14 h-14 shrink-0 bg-slate-100 rounded-2xl flex items-center justify-center group-hover:bg-blue-600 transition-all duration-300 group-hover:shadow-lg group-hover:shadow-blue-200">
                                  {type.icon ? (
                                    <img src={type.icon} alt="" className="w-7 h-7 object-contain group-hover:brightness-0 group-hover:invert transition-all" />
                                  ) : (
                                    <Zap size={22} className="text-slate-400 group-hover:text-white transition-colors" />
                                  )}
                                </div>
                                <div className="flex flex-col">
                                  <span className="text-[15px] font-bold text-slate-800 group-hover:text-blue-600 transition-colors leading-tight">{type.name}</span>
                                  <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter mt-1">Explore Type</span>
                                </div>
                              </Link>
                            ))
                          ) : (
                            <div className="col-span-2 py-10 text-center text-slate-400">No active types found.</div>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              {!initialized ? (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-black text-slate-800 leading-none">{authUser.name}</p>
                      <p className="text-[11px] text-slate-400 mt-0.5 capitalize">{authUser.role}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link href={getDashboardRoute()} onClick={() => setOpenMobile(false)}>
